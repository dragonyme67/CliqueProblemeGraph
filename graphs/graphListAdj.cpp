...
gint GraphAdjVector::degree(vertex v) const { return _neighbors_set[v].size(); }
...
{
    for (auto it = _neighbors_set[v1].begin() ; it != _neighbors_set[v1].end();
++it ){ if (*it==v2){ *it = _neighbors_set[v1].back();
...
void GraphAdjVectorSorted::intersect_neighbors(vector<vertex> &vect,
  ...
  std::set_intersection(vect.begin(), vect.end(), neigh_v1.begin(),
                        neigh_v1.end(), std::back_inserter(result));
...
void GraphHeavy::compute_adjency_matrix() {
  ...
  for (vertex v(0); v < nb_vertices(); v++)
    for (auto v2 : _neighbors_set[v])
      if (v2 > v)
        _adjMat->add_edge(v, v2);
}
