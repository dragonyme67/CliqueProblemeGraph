...
public:
  ...
  virtual void add_edge(vertex r, vertex c) {
    _matrix[_n * r + c] = true;
    _matrix[_n * c + r] = true;
  };
  virtual void delete_edge(vertex r, vertex c) {
    _matrix[_n * r + c] = false;
    _matrix[_n * c + r] = false;
  };
  // unsigned int size_of(){return sizeof(_matrix) + sizeof(_n);};
};
...
public:
  AdjSymTrig(vertex n) : _n(n), _matrix(n * (n + 1) / 2) {
    _matrix.shrink_to_fit();
  };
  gint nb_vertex() { return _n; };
  bool is_edge(vertex r, vertex c) {
    vertex mi = std::min(r, c);
    vertex ma = std::max(r, c);
    return _matrix[(ma * (ma + 1)) / 2 + mi];
  };
  void add_edge(vertex r, vertex c) {
    vertex mi = std::min(r, c);
    vertex ma = std::max(r, c);
    _matrix[(ma * (ma + 1)) / 2 + mi] = true;
  };
  void delete_edge(vertex r, vertex c) {
    vertex mi = std::min(r, c);
    vertex ma = std::max(r, c);
    _matrix[(ma * (ma + 1)) / 2 + mi] = false;
  };
...
public:
  AdjSymRectDeg(vertex n) : AdjSymRect(n), _degrees(n) {
    _degrees.shrink_to_fit();
  };
