...
/*
vertex MaxWeightStrategy::select(const Graph &g, const std::vector<vertex>
&restants) const { vertex best_v = (vertex)-1; gint max_w = 0; for (vertex v :
restants) { gint w = getVertexWeight(g, v); // [cite: 44] if (w > max_w) { max_w
= w; best_v = v;
        }
    }
    return best_v;
