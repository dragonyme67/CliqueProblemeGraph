 ...
 * stratégie : Best Improvement (max somme degrés résiduels) ou First
 * Improvement.
 */
bool move_n2_add(const Graph &g, std::vector<vertex> &clique,
                 std::vector<vertex> &candidats, bool best_improvement);

/**
 * Tente d'ajouter un seul sommet à la clique (Repli N1).
 */
bool move_n1_add(const Graph &g, std::vector<vertex> &clique,
 ...
 */
bool move_n2_swap(const Graph &g, std::vector<vertex> &clique,
                  std::vector<vertex> &candidats);

/**
 * Perturbation par destruction aléatoire d'une partie de la clique.
 */
void perturb_destruction(const Graph &g, std::vector<vertex> &clique,
                         std::vector<vertex> &candidats, double rate);
